...
class Fx extends dn.Process {
	...
	// Green lazer tweet 08-04-20
	public function backrocket(x:Float, y:Float, c:UInt, a=1.0) {
		var p = allocTopAdd(getTile("shootLine"), x,y+70);
		p.setScale(rnd(2,3));
		p.setFadeS(a, 0, 0.3);
		p.colorize(c);
		p.ds = 0.1;
	...
	public function shotblast(x:Float, y:Float, c:UInt, a=1.0) {
		var p = allocTopAdd(getTile("shotBlast"), x,y);
		p.setScale(1);
		//p.setFadeS(a, 0, 0.3);
		p.colorize(c);
		//p.ds = 0.1;
	...
	public function fxsmallcircle(x:Float, y:Float, c:UInt, a=1.0) {
		var p = allocTopAdd(getTile("smallcircle"), x,y);
		p.setScale(rnd(2,3));
		p.setFadeS(a, 0, 0.3);
		p.colorize(c);
		p.ds = 0.1;
	...
	public function fxpixel(x:Float, y:Float, c:UInt, a=1.0) {
		var p = allocTopAdd(getTile("pixel"), x,y);
		p.setScale(rnd(2,3));
		p.setFadeS(a, 0, 0.3);
		p.colorize(c);
		p.ds = 0.1;
